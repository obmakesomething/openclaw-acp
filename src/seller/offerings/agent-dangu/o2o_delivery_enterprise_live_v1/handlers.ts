...
const OFFERING_NAME = "o2o_delivery_enterprise_live_v1";
...
export async function executeJob(request: any): Promise<ExecuteJobResult> {
  const storeInfo = await callPrintful("/store");
  ...
  const deliverable = {
    ...
    guidance: {
      ...
      enterprise: "For enterprise fulfillment, execute order creation in a controlled backend worker with idempotency keys.",
    },
