...
const OFFERING_NAME = "o2o_delivery_pro_live_v1";
const PLAN = "pro";
...
export async function executeJob(request: any): Promise<ExecuteJobResult> {
  ...
  const normalizedItems = Array.isArray(request?.items)
    ? request.items.filter(
        (item: any) =>
          Number.isFinite(Number(item?.variant_id ?? item?.variantId)) &&
          Number(item?.quantity ?? 1) > 0
      )
    : [];
  const addressRisk =
    request?.countryCode?.toUpperCase?.() === "US" && !request?.stateCode
      ? "medium"
      : !request?.countryCode || !request?.postalCode
        ? "high"
        : "low";
  ...
  const deliverable = {
    ...
    diagnostics: {
      inputCompleteness: {
        hasCountryCode: Boolean(request?.countryCode),
        hasPostalCode: Boolean(request?.postalCode),
        hasItems: normalizedItems.length > 0,
      },
      normalizedItemCount: normalizedItems.length,
      addressRisk,
      suggestedUpgrade:
        ratesPreview?.ok && addressRisk !== "high"
          ? "o2o_fulfillment_readiness_enterprise_v1"
          : "Submit full destination + item payload and retry pro validation",
    },
    guidance: {
      quote:
      ...
      enterprise:
        "For high-volume operations, run o2o_fulfillment_readiness_enterprise_v1 before production order execution.",
    },
