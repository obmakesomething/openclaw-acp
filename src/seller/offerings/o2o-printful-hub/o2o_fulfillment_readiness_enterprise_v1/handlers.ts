...
const OFFERING_NAME = "o2o_fulfillment_readiness_enterprise_v1";
...
export async function executeJob(request: any): Promise<ExecuteJobResult> {
  ...
  const productsPreview = await callPrintful("/store/products?limit=25");
  ...
  if (!PRINTFUL_TOKEN) riskFlags.push("missing_printful_credentials");
  if (!ratePayload) riskFlags.push("missing_rate_inputs");
  if (ratePayload && ratesPreview && !ratesPreview.ok) riskFlags.push("shipping_rate_api_error");
  if (request?.countryCode?.toUpperCase?.() === "US" && !request?.stateCode) {
    riskFlags.push("us_state_code_missing");
  }
  const readinessScore = Math.max(0, 100 - riskFlags.length * 25);
  ...
  const deliverable = {
    ...
    requestEcho: {
      ...
      monthlyOrderVolume: request?.monthlyOrderVolume || null,
      idempotencyRequired: request?.idempotencyRequired ?? null,
    },
    enterpriseReadiness: {
      readinessScore,
      riskFlags,
      controls: {
        ...
        dryRunFirst: true,
        manualApprovalForLargeBatches: true,
      },
    },
    guidance: {
      quote:
        "Provide items with variant_id and quantity plus countryCode/postalCode for live shipping rates.",
      order:
        "Order creation is intentionally not executed in this handler to avoid unintended fulfillment.",
      enterprise:
        "For enterprise fulfillment, execute order creation in a controlled backend worker with idempotency keys.",
      fallback:
        "If readiness score is low, run o2o_delivery_pro_live_v1 to resolve payload and address risks first.",
    },
    status: !PRINTFUL_TOKEN
      ? "blocked"
      : ratesPreview?.ok && readinessScore >= 75
        ? "enterprise_ready"
        : ratePayload
          ? "degraded"
          : "needs_input",
